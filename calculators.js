...
function calculateRetirement(currentAge, retirementAge, currentSavings, monthlyContribution, 
  ...
  if (monthlyRate === 0) {
    return { corpusAtRetirement: 0, totalWithdrawn: 0, yearsOfRetirement: 0, message: 'Invalid retirement age' };
  }
  ...
  const withdrawalRate = annualReturn / 12 / 100;
  
  // Calculate how much can be withdrawn during retirement
  let balance = corpus;
  let totalWithdrawn = 0;
  
  for (let i = 0; i < retirementYears * 12 && balance > 0; i++) {
    const monthlyWithdrawal = balance * withdrawalRate;
